...
#define lvector_resize(vector, newsize)                                        \
  ({                                                                           \
    ...
    if (newsize == 0) {                                                        \
      ...
      for (size_t __lvector_resize_iterator = newsize;                         \
           __lvector_resize_iterator < (vector).len;                           \
           ++__lvector_resize_iterator) {                                      \
...
#define lvector_assign(vector_dest, vector_src, copier)                        \
  ({                                                                           \
    int __lvector_assign_ret = 0;                                              \
                                                                               \
    __lvector_assign_ret = lvector_create(                                     \
        vector_dest, lvector_size(vector_src), (vector_src).destr);            \
    if (__lvector_assign_ret == 0) {                                           \
      if (copier != NULL) {                                                    \
        size_t __lvector_assign_iterator = 0;                                  \
        lvector_foreach(obj, vector_src) {                                     \
          __lvector_assign_ret =                                               \
              copier(&(vector_dest).arr[__lvector_assign_iterator], obj);      \
          if (__lvector_assign_ret != 0) {                                     \
        ...
        for (size_t __lvector_assign_iterator = 0;                             \
             __lvector_assign_iterator < lvector_size(vector_src);             \
             ++__lvector_assign_iterator) {                                    \
  ...
  ({                                                                           \
    ...
    if (position < lvector_size(vector)) {                                     \
      ...
      if (__lvector_insert_ret == 0 && (vector).len < (vector).rsize) {        \
        memmove(&(vector).arr[position + 1], &(vector).arr[position],          \
                (lvector_size(vector) - position) *                            \
...
#define lvector_erase_item(vector, item)                                       \
  {                                                                            \
    for (size_t __lvector_erase_item_iterator = 0;                             \
         __lvector_erase_item_iterator < lvector_size(vector);                 \
         ++__lvector_erase_item_iterator)                                      \
...
#define lvector_erase_if(vector, condition_function, ...)                      \
  ({                                                                           \
    size_t __lvector_erase_if_original_count = lvector_size(vector);           \
                                                                               \
    for (size_t __lvector_erase_if_iterator = 0;                               \
         __lvector_erase_if_iterator < lvector_size(vector);) {                \
      if (condition_function(&(vector).arr[__lvector_erase_if_iterator],       \
                             ##__VA_ARGS__) == 0) {                            \
...
#define lvector_find_if(vector, condition_function, ...)                       \
  ({                                                                           \
    __typeof__(*(vector.arr)) *__lvector_find_if_item_found = NULL;            \
                                                                               \
    for (size_t __lvector_find_if_iterator = 0;                                \
         __lvector_find_if_iterator < lvector_size(vector);                    \
         ++__lvector_find_if_iterator) {                                       \
      if (condition_function(&(vector).arr[__lvector_find_if_iterator],        \
                             ##__VA_ARGS__) == 0) {                            \
        __lvector_find_if_item_found =                                         \
            &(vector).arr[__lvector_find_if_iterator];                         \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
    __lvector_find_if_item_found;                                              \
...
#define lvector_macro_find_if(vector, item_name, condition)                    \
  ({                                                                           \
    __typeof__(*(vector.arr)) *__lvector_macro_find_if_item_found = NULL;      \
                                                                               \
    for (size_t __lvector_macro_find_if_iterator = 0;                          \
         __lvector_macro_find_if_iterator < lvector_size(vector);              \
         ++__lvector_macro_find_if_iterator) {                                 \
      __typeof__(*(vector.arr)) *item_name =                                   \
          &(vector).arr[__lvector_macro_find_if_iterator];                     \
      ;                                                                        \
      if (condition) {                                                         \
...
#define lvector_clear(vector)                                                  \
  {                                                                            \
    if ((vector).destr != NULL) {                                              \
      lvector_foreach(item, vector) { (vector).destr(item); }                  \
    }                                                                          \
    (vector).len = 0;                                                          \
  }
...
#define lvector_spush_back(vector, new_item)                                   \
  ({                                                                           \
    ...
    if (lvector_size(vector) == 0) {                                           \
      ...
      for (size_t __lvector_spush_back_iterator = 0;                           \
           __lvector_spush_back_iterator < lvector_size(vector);               \
           ++__lvector_spush_back_iterator) {                                  \
        if ((vector).arr[__lvector_spush_back_iterator] == new_item) {         \
          __lvector_spush_back_ret = 0;                                        \
          break;                                                               \
        } else if (__lvector_spush_back_iterator ==                            \
                       lvector_size(vector) - 1 &&                             \
                   (vector).arr[__lvector_spush_back_iterator] != new_item) {  \
          __lvector_spush_back_ret = lvector_push_back(vector, new_item);      \
...
#define lvector_for(index_name, vector)                                        \
  for (size_t index_name = 0; index_name < lvector_size(vector); ++index_name)
